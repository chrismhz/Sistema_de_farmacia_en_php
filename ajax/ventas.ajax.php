...
class AjaxVentas{
    ...
    public function AjaxGenerarTicketVenta($nro_boleta){
        ...
        $pdf->Cell(60, 6, utf8_decode('~° FARMACIAS DEL AHORRO °~'),1,0,'C');

        $pdf->Ln();
        $pdf->SetFont('Arial', '', 8);
        $pdf->Cell(60, 6, "RUC 17283945068",0,0,'C'); 
        $pdf->Ln();
        $pdf->SetFont('Arial', '', 8);
        $pdf->Cell(60, 6, "Emiliano Zapata 459 Col. Cruz de Dendho",0,0,'C');
        $pdf->Ln();
        $pdf->SetFont('Arial', '', 8);
        $pdf->Cell(60, 6, "TEL. 722-261-5244",0,0,'C');
        $pdf->Ln();
        $pdf->SetFont('Arial', '', 8);
        $pdf->Cell(60, 6, utf8_decode("Jilotepec de Molina Enriquez - Edo. De México"),0,0,'C');     
        
        $pdf->Ln();

        $pdf->SetFont('Arial', 'B', 9);
        $pdf->Cell(60, 8, utf8_decode("TICKET DE VENTA"),0,0,'C');
        ...
        $pdf->Ln();
        $pdf->Cell(60, 5, utf8_decode("________________________________"),0,0,'C');
        $pdf->Ln();
        ...
        foreach ($productos as $pro) {
            ...
            $pdf->Cell(15, 4, "$ " . number_format($pro["total_venta"], 2, ".", ","), 0, 0, 'R');

            $pdf->Cell(25, 5,  "$ " . number_format($pro["cantidad"] * $pro["total_venta"], 2, ".", ","), 0, 0, "R");            

            $pdf->Ln();

            $total += $pro["cantidad"] * $pro["total_venta"];
                        
        }

        $pdf->Ln();        
        $pdf->Cell(20, 4, "Efectivo.:" );
        $pdf->Cell(40, 4, "$ ". ROUND($efectivo,2),0, 0, 'R');
        $pdf->Ln();

        $pdf->Cell(20, 4, "TOTAL A PAGAR: ");
        $pdf->Cell(40, 4, "$ ". ROUND($total,2),0, 0, 'R');
        $pdf->Ln();

        $pdf->Ln();        
        $pdf->Cell(20, 4, "CAMBIO.:" );
        $pdf->Cell(40, 4, "$ ". ROUND($cambio,2),0, 0, 'R');
        $pdf->Ln();
